...
def plot_attentions_all_layer(self, current_state):
    ...
    if 'bert_glue_trainer' in self.trainer_name: # base_model
        ...
        sys.exit(f"{self.trainer_name} is neither bert_glue_trainer nor perlin_trainer.")
    
    os.makedirs(f'./saves/trainer/{self.trainer_name}/{viz_matrix_type}/', exist_ok=True) # TODO check directory..
    ...
    for b in range(_batch_size_for_viz):
        ...
        _attention_mask_idx = self.viz_batch['attention_mask'][self.batch_index].shape[0] # inlcude padding
        for l in range(_layer_num):
            ...
            plt.tight_layout()
            plt.show()
            _saved_path = f'./saves/trainer/{self.trainer_name}/{viz_matrix_type}/{_batch_idx+1}_{l+1}.png'
